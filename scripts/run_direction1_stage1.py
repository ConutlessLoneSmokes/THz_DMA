...
from thz_dma.stage1 import run_stage1_los


def main() -> int:
    config_path, config, requested_run_id = parse_run_arguments(
        "direction1_stage1_los.toml", __doc__
    )
    require_unit_pilot_energy(
        config["resources"]["pilot_energy_per_re"], "Stage 1-A"
    )
    archive = create_run_archive(
        ...
        checkpoint_note="No learned model is used in Stage 1-A.\n",
    ...
    try:
        ...
        result_path = archive.write_result("direction1_stage1", "summary", summary)
