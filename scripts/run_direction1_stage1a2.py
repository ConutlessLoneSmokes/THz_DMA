...
from thz_dma.stage1a2 import run_stage1a2  # noqa: E402
...
def _write_raw_metrics(path: Path, rows: list[dict]) -> None:
    if not rows:
        raise ValueError("Stage 1-A2 produced no metric rows")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        / "configs"
        / "direction1_stage1a2_gain_control.toml",
    )
        ...
        json.dumps(
            {
                ...
                "dma_candidate_seed_start": config["dma"][
    ...
    (checkpoint_dir / "README.txt").write_text(
        "No learned model is used in Stage 1-A2.\n", encoding="utf-8"
    )
    ...
    try:
        ...
        (run_dir / "execution_schedule.json").write_text(
            json.dumps(
                summary["execution_schedule"], indent=2, ensure_ascii=False
            ),
            encoding="utf-8",
        )
        results_dir = PROJECT_ROOT / "results" / "direction1_stage1a2"
