...
from thz_dma.analysis.stage1b import analyze_stage1b  # noqa: E402
from thz_dma.stage1b import run_stage1b  # noqa: E402
...
def _write_raw_metrics(path: Path, rows: list[dict]) -> None:
    if not rows:
        raise ValueError("Stage 1-B produced no metric rows")
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "direction1_stage1b_twopath.toml",
    )
    ...
    (checkpoint_dir / "README.txt").write_text(
        "No learned model is used in Stage 1-B.\n", encoding="utf-8"
    )
    ...
    try:
        ...
        compact = analyze_stage1b(run_dir)
        results_dir = PROJECT_ROOT / "results" / "direction1_stage1b"
    ...
    _write_status(
        ...
        decision_status=compact["primary_gate"]["status"],
    ...
    print(f"decision: {compact['primary_gate']['status']}", flush=True)
    return 0
