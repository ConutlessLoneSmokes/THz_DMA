...
更新日期：2026-09-14  
状态：100 场景正式平台验证通过；Stage 2-A 条件诊断中 F2 通过、F1 保留局部角度歧义，正式比较暂停
...
3. 暂不设计未知全局几何搜索。先在现有场景分解 F1 真值峰—错误峰及单径/三径差异；F1 条件恢复稳定后，再确定全局获取和 Stage 2-A 正式配对，并接入 Deshpande 类单次频率解码和 Zhang 类接收侧物理感知张量分解。
4. 统一输出路径/CSI 指标、可行数据态波束损失、扣除训练与切换后的净谱效率和批量 1 时间。
5. 经典方法比较完成且仍存在稳定精度—在线时间缺口后，再把 DMA 扩展方法升级到轻量展开学习；当前不加入 AI/深度学习。
